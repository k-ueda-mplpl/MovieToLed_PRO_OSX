...
void DeviceMap::create(std::string product_name, uint16_t num_product, uint16_t num_device, ProductProfile::DeviceType device_type, ProductProfile::DeviceIdFormat id_format) {
	// DeviceMap.mdの削除
	...
	if (device_type == ProductProfile::DeviceType::LINE4) {
		out << "- Device Type: 4 Line Device (incl. POLE and RIBBON Device)\n";
	} else if (device_type == ProductProfile::DeviceType::LINE8) {
		out << "- Device Type: 8 Line Device (incl. POMPOM Device(PB))\n";
	}
	if (id_format == ProductProfile::DeviceIdFormat::HEX) {
		out << "- ID Format: HEX(16)\n";
	} else if (id_format == ProductProfile::DeviceIdFormat::DEC) {
	...
	if (max_product_count > 0) {
		...
		for (uint16_t product_id = 0; product_id < num_product; product_id++) {
			...
			for (uint16_t device_id = 0; device_id < num_device; device_id++) {
				// ディレクトリ名
				if (id_format == ProductProfile::DeviceIdFormat::HEX && (device_id & 0xff) == 0) {
					snprintf(dir_name, 12, "%03X-%03X-HEX", product_id, device_id);
				} else if (id_format == ProductProfile::DeviceIdFormat::DEC && device_id % 100 == 0) {
...
void DeviceMap::create(const ProductProfile & profile) {
	create(profile.product_name, profile.num_product, profile.num_device, profile.device_type, profile.device_id_format);
}